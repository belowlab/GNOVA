...
class PlinkBEDFile(__GenotypeArrayInMemory__):
    ...
    def nextSNPs(self, b, minorRef=None):
        ...
        X = X[0:n, :]
        # Compute normalized genotypes
        # Y = np.zeros(X.shape)
        # ii = X != 9
        # avg = np.mean(X[ii], axis=0)
        # X[np.logical_not(ii)] = avg
        # denom = np.std(X, axis=0)
        # denom[denom == 0] = 1

        # if minorRef is not None:
        #     minor_allele_freq = self.freq[self._currentSNP:self._currentSNP + b]
        #     denom[minor_allele_freq > 0.5] *= -1
    ...
    #     if minorRef is not None:
    #         minor_freq = self.freq[self._currentSNP : self._currentSNP + b]
    #         flip_ref = minor_freq > 0.5
    #         X[:, flip_ref] = -X[:, flip_ref]
